...
if __name__ == '__main__':
    ...
    all_dir = sys.argv[1]
    output_dir = './submit/smooth/'+all_dir[:-4]+'.png'
    ...
    if env == 'atari':
        ...
        data_path = '/lab/tmpig10c/kiran/Dropbox/logs/rllib/atari/notemp/'
    else:
    ...
    for name,i in game_list.items():
        ...
        values=[]
        max_len=0
        if 'union' in all_dir[:-4] or 'wall' in all_dir[:-4]:
            if exp=='e2e':
        ...
        cs = {'E2E':'grey', 'TCN+':'#2ca02c', 'TCN':'#2ca02c','VEP':'#ff7f0e', 'VIP':'#9467bd', 'SOM':'#d62728', 'RANDOM':'#1f77b4', 'VEP(.25)':'#964B00', 'VEP(1)':'#a02ca0'}
        if exp=='e2e':
            ...
            ax = sns.lineplot(data=data,x='Compute Time (in minutes)', y='Reward', color=cs[name], linewidth=2.5)
            ...
            y_max = data['Reward'].max()  # Calculate the maximum y value in your data
            # ax.set_ylim(0, data['Reward'].quantile(0.95))
            # ax.tight_layout()
            ax.margins(x=0, y=0)
            ax.plot()
        else:
            ...
            data = pd.DataFrame(data)
            ax = sns.lineplot(data=data,x='Iterations', y='Reward', color=cs[name], linewidth=2.5)
            ax.tick_params(axis='both', which='major', labelsize=17)
            ax.set(xlabel=None, ylabel=None, title=None)
            ax.legend().set_visible(False)
            ax.spines['top'].set_visible(False)
