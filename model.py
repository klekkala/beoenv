...
TORCH_GLOBAL_SHARED_BACKBONE= VAE(channel_in=3, ch=32, z=512)
...
class SharedAtariModel(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        if 'backbone' in args.setting:
            self.backbone = TORCH_GLOBAL_SHARED_BACKBONE
        
        if 'policy' in args.setting:
...
class SharedWeightsModel(TorchModelV2, nn.Module):
