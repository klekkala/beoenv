...
class Decoder(nn.Module):
    ...
    def __init__(self, channels, ch=32, z=32):
        ...
        size=84
        sc = math.pow(size, 1 / 6)
        size_list=[size]
        for i in range(1,6):
            size_list.append(round(size_list[i-1]/sc))
        size_list.append(1)
        scale_list=[]
        for i in range(6):
            scale_list.append(size_list[i]/size_list[i+1])

        self.conv1 = ResUp(z, ch*8, scale_list[5])
        self.conv2 = ResUp(ch*8, ch*8, scale_list[4])
        self.conv3 = ResUp(ch*8, ch*4, scale_list[3])
        self.conv4 = ResUp(ch*4, ch*2, scale_list[2])
        self.conv5 = ResUp(ch*2, ch, scale_list[1])
        self.conv6 = ResUp(ch, ch//2, scale_list[0])
...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.flatten(encoding, start_dim=1)
        #return recon, torch.flatten(encoding, start_dim=1), log_var
