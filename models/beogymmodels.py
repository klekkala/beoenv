...
class ComplexNet(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        if num_outputs:
            ...
            self.logits_layer = SlimFC(
                in_size=512,  # Input size should match the output size of the previous layer
                out_size=num_outputs,
...
class SingleImageModel(VisionNetwork):
    ...
    def __init__(
        ...
        if  model_config['custom_model_config']['backbone'] == 'vep':
            #put the clip code here
            #self._convs = clipmodel
            #self._model = 'clip'
            #self._convs, self._pre = clip.load('RN50',device = 'cuda')
            print('its vep')
            self._convs = TColEncoder(channel_in=chan_in, ch=32, z=512)
            checkpoint = torch.load('/lab/kiran/ckpts/pretrained/colo/3CHAN_VEP_COLL_3CHAN_OBJCOLOR_TABTEXT_STANDARD_2.0_0.01_2_nsame_triplet_32_2_0.0001_1.pt', map_location="cpu")
            self._convs.load_state_dict(checkpoint['model_state_dict'])
        else:
            self._model = 'not'
            chan_in = 3
